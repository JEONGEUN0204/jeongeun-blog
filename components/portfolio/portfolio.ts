...
export type DetailSection = IndexItem & {
...
export type PortfolioProduct = {
  ...
  no: string
  name: string
  /** 제품이 놓인 자리. content/products.ts 가 원천이라 항상 있다. */
  platform: string
  summary?: string
  /** 주력 스택. lead 타일은 전부, wide 타일은 앞의 두 개를 앞면에 올린다. */
  stack: string[]
  /** 타일 스크린샷. 이미지가 없는 제품은 글자만 싣는다. */
  cover?: ProjectCover
  /** 헤더·스크린샷·요약. 본문 맨 앞에 온다. */
  overview: ReactNode
  /** 제품 안의 작업 단위 섹션. 제목이 카드 뒷면과 목차에 그대로 올라간다. */
 ...
 * 첫 타일은 2×2(lead), 둘째는 가로 2칸(wide), 나머지는 한 칸이다. 제품이 넷이면 4열 2행이 빈칸 없이 찬다.
 */
export const tileSize = (index: number): TileSize =>
  index === 0 ? 'lead' : index === 1 ? 'wide' : 'small'

export const cardId = (productId: string) => `card-${productId}`
export const detailId = (productId: string) => `detail-${productId}`
/** 본문 맨 앞(개요) 자리. 목차의 첫 줄이 가리킨다. */
export const overviewId = (productId: string) => `${productId}-overview`
